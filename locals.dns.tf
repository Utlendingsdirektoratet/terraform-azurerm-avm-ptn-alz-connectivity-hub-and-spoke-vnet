...
locals {
  ...
  private_dns_zones_auto_registration = { for key, value in var.hub_virtual_networks : key => {
    ...
    parent_id   = provider::azapi::subscription_resource_id(data.azurerm_subscription.current.subscription_id, local.resource_group_resource_type, [local.private_dns_zones[key].resource_group_name])
